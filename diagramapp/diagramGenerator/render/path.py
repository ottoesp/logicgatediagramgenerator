...
class Gutter:
    ...
    def add_path(self, start_id : str, dest_id : str):
        ...
        lane = self.lanes[start_node.get_id()]
        if lane is None:
            self.lanes[start_node.get_id()] = self.next_lane
            lane = self.next_lane
            self.next_lane += 1
        ...
        for y in range(lane_y):
            self.grid[start_node.x][y].add(start_node.get_id())
        ...
        if (start_node.x < dest_node.x):
             for x in range(start_node.x, dest_node.x, 1):
                 self.grid[x][lane_y].add(start_node.get_id())
        else:
            for x in range(start_node.x, dest_node.x, -1):
                 self.grid[x][lane_y].add(start_node.get_id())

        # Draw end, scanning horizontally from lane to dest node
        for y in range(lane_y, dest_node.y):
            self.grid[dest_node.x][y].add(start_node.get_id())
